...
- `jsonls`/`yamlls` offline schema fetches are gated by three load-bearing pieces in `lua/yyxi/plugins/language_tools.lua` plus the `vscode/content` handler at `lua/yyxi/lsp/schema_content_handler.lua`: jsonls `init_options.handledSchemaProtocols = { 'file' }`, the `yaml/registerContentRequest` notification sent from yamlls `on_init`, and the shared handler that resolves schemastore URLs against the vendored tree. The `schemaDownload.enable = false` setting jsonls accepts is a no-op — `vscode-json-language-server` does not read it. Removing any of the other three re-enables DNS lookups for `$schema` URLs declared inside user documents.
...
- For a new language, keep filetype detection and Tree-sitter alias policy in `lua/yyxi/plugins/filetypes.lua`, keep parser/query pinning in `source-manifest.json`, prefer minimal grammar-source `files` entries that include only build-required paths, and keep parser/query sources aligned when the grammar ships its own queries or uses a released-source exception. Update the supplemental `textobjects` language allowlist only when `nvim-treesitter-textobjects` is the actual query source for that parser. Neovim should consume the managed Tree-sitter runtime rather than install anything itself.
- For any `source-manifest.json` commit update or new source, treat the task as supply-chain review first: fetch or clone the exact candidate revision, diff it against the locked commit when applicable, inspect new build/runtime/network/dynamic-exec surface, verify the stated `ref` contains the commit, and run `./manage scan --strict <worktree-or-tempdir>` before any Neovim invocation that could load managed dependency code.
- For `source-manifest.json` metadata-only changes with no commit change, skip fetch/provenance work but still review `files`, `allowedDirtyPaths`, `treesitter`, and `plugin` changes as trust-surface changes.
