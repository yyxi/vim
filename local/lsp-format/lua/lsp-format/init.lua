...
M.setup = function(format_options)
  M.format_options =
    vim.tbl_deep_extend('force', M.format_options, format_options or {})

  -- vim.api.nvim_create_user_command("LspFormat", M.format, { nargs = "*", bar = true, force = true })
...
M.format = function()
  if vim.b.format_saving or M.disabled_filetypes[vim.bo.filetype] then
    return
  end

  local bufnr = util.validate_bufnr(vim.api.nvim_get_current_buf() or 0)
  ...
  -- for _, _ in pairs(format_options.order or {}) do
  ...
  if #clients > 0 then
    table.insert(
      M.queue,
      { bufnr = bufnr, clients = clients, format_options = format_options }
    )
    M._next()
  end
end
...
M._handler = function(err, result, ctx)
  if err ~= nil then
    ...
    local client_name = client and client.name
      or string.format('client_id=%d', ctx.client_id)
...
-- M._organize = function(bufnr, client)
--     if not client.commands or not client.commands.LspOrganizeImports then return end
--     client.commands.LspOrganizeImports[1](bufnr, client)
-- end

M._extra = function(bufnr, client)
  if not client.config or not client.config.format then
    return
  end

  for _, f in pairs(client.config.format) do
...
M._format = function(bufnr, client, format_options)
  ...
  M._handler(
    result.err,
    result.result,
    { client_id = client.id, bufnr = bufnr, params = params }
  )
end
...
M._next = function()
  ...
  M._extra(next.bufnr, next_client)
