...
M.setup = function(server_name, user_opts)
  if state.exclude[server_name] then
    return
  end
...
s.set_keymaps = function(bufnr)
  ...
  -- map('n', 'xf', 'gf')
  map('n', 'xd', lsp('buf.definition()'))
  map('n', 'xD', lsp('buf.declaration()'))
  map('n', 'xi', lsp('buf.implementation()'))
  map('n', 'xo', lsp('buf.type_definition()'))
  map('n', 'xR', lsp('buf.references()'))
  map('n', 'xr', lsp('buf.rename()'))
  map('n', 'xa', lsp('buf.code_action()'))
  map('x', 'xa', lsp('buf.code_action()'))
...
s.use_cmp = function(current)
  if state.capabilities == nil then
    ...
    if ok then
      ...
      local msg =
        'Could not find cmp_nvim_lsp. Please install cmp_nvim_lsp or set the option cmp_capabilities to false (use set_preferences).'
