...
local run = function(args)
  ...
  if suggest then
    ...
    if handle_setup == 'per-project' then
      return
    end
  end
...
M.use = function(servers, lsp_opts, force)
  ...
  for _, name in pairs(servers) do
    local opts =
      vim.tbl_deep_extend('force', {}, internal.servers[name] or {}, lsp_opts)
...
M.suggest_server = function()
  ...
  local is_there = util.should_suggest_server(ft, installer.fn.get_servers())
  ...
  local server_available =
    util.should_suggest_server(ft, util.available_servers())
